...
public class SchemaChangesTest extends CCMTestsSupport {
  ...
  /**
   * The maximum time that the test will wait to check that listeners have been notified. This
   * threshold is intentionally set to a very high value to allow CI tests to pass.
   */
  ...
  public void teardown() {
    if (cluster1 != null) cluster1.close();
    if (cluster2 != null) cluster2.close();
    if (schemaDisabledCluster != null) schemaDisabledCluster.close();
  }
  ...
  /**
   * Ensures that a listener registered on a Cluster that has schema metadata disabled is never
   * invoked with schema change events.
   *
  ...
  public void should_notify_of_table_update(String keyspace) throws InterruptedException {
    ...
    for (SchemaChangeListener listener : listeners) {
      ...
      assertThat(previous.getValue()).isEqualTo(added.getValue()).hasNoColumn("j");
  ...
  public void should_retain_view_on_table_update(String keyspace) throws InterruptedException {
    ...
    for (SchemaChangeListener listener : listeners) {
      ArgumentCaptor<MaterializedViewMetadata> viewAdded =
          ArgumentCaptor.forClass(MaterializedViewMetadata.class);
      verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
          .onMaterializedViewAdded(viewAdded.capture());
      assertThat(viewAdded.getValue()).hasName("mv1");
    }
    for (Metadata m : metadatas()) {
    ...
    // Alter table, ensure event is received and metadata is updated.  Most importantly, ensure view
    // is
  ...
  public void should_notify_of_table_drop(String keyspace) throws InterruptedException {
    ...
    for (SchemaChangeListener listener : listeners) {
      ArgumentCaptor<TableMetadata> added = ArgumentCaptor.forClass(TableMetadata.class);
      verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1)).onTableAdded(added.capture());
      assertThat(added.getValue()).hasName("table1").isInKeyspace(handleId(keyspace));
    }
    ...
    for (Metadata m : metadatas()) assertThat(m.getKeyspace(keyspace).getTable("table1")).isNull();
  }
  ...
  public void should_notify_of_function_update(String keyspace) {
    ...
    session1.execute(
        String.format(
            "CREATE OR REPLACE FUNCTION %s.\"ID\"(i int) RETURNS NULL ON NULL INPUT RETURNS int LANGUAGE java AS 'return i + 1;'",
            keyspace));
    for (SchemaChangeListener listener : listeners) {
      ...
      verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
          .onFunctionChanged(current.capture(), previous.capture());
  ...
  public void should_notify_of_aggregate_creation(String keyspace) {
    session1.execute(
        String.format(
            "CREATE FUNCTION %s.\"PLUS\"(s int, v int) RETURNS NULL ON NULL INPUT RETURNS int LANGUAGE java"
                + " AS 'return s+v;'",
            keyspace));
    session1.execute(
        String.format(
            "CREATE AGGREGATE %s.\"SUM\"(int) SFUNC \"PLUS\" STYPE int INITCOND 0;", keyspace));
    for (SchemaChangeListener listener : listeners) {
      ...
      assertThat(added.getValue()).isInKeyspace(handleId(keyspace)).hasSignature("\"SUM\"(int)");
    }
    for (Metadata m : metadatas())
      assertThat(m.getKeyspace(keyspace).getAggregate("\"SUM\"", DataType.cint())).isNotNull();
  }
  ...
  public void should_notify_of_aggregate_update(String keyspace) {
    ...
    session1.execute(
        String.format(
            "CREATE OR REPLACE AGGREGATE %s.\"SUM\"(int) SFUNC \"PLUS\" STYPE int INITCOND 1",
            keyspace));
    for (SchemaChangeListener listener : listeners) {
      ...
      verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
          .onAggregateChanged(current.capture(), previous.capture());
  ...
  public void should_notify_of_aggregate_drop(String keyspace) {
    ...
    for (SchemaChangeListener listener : listeners) {
      ...
      assertThat(removed.getValue()).isInKeyspace(handleId(keyspace)).hasSignature("\"SUM\"(int)");
    }
    for (Metadata m : metadatas())
      assertThat(m.getKeyspace(keyspace).getAggregate("\"SUM\"", DataType.cint())).isNull();
  }
  ...
  public void should_notify_of_view_update(String keyspace) {
    ...
    session1.execute(
        String.format(
            "ALTER MATERIALIZED VIEW %s.mv1 WITH compaction = { 'class' : 'LeveledCompactionStrategy' }",
            keyspace));
    for (SchemaChangeListener listener : listeners) {
      ArgumentCaptor<MaterializedViewMetadata> current =
          ArgumentCaptor.forClass(MaterializedViewMetadata.class);
      ArgumentCaptor<MaterializedViewMetadata> previous =
          ArgumentCaptor.forClass(MaterializedViewMetadata.class);
      verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
  ...
  public void should_notify_of_view_drop(String keyspace) {
    ...
    for (SchemaChangeListener listener : listeners) {
      ArgumentCaptor<MaterializedViewMetadata> removed =
          ArgumentCaptor.forClass(MaterializedViewMetadata.class);
      verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
          .onMaterializedViewRemoved(removed.capture());
      assertThat(removed.getValue()).hasName("mv1");
    }
    for (Metadata m : metadatas())
  ...
  public void should_notify_of_keyspace_update(String keyspace) throws InterruptedException {
    ...
    assert added != null;
    for (Metadata m : metadatas()) assertThat(m.getKeyspace(keyspace).isDurableWrites()).isTrue();
    ...
    for (SchemaChangeListener listener : listeners) {
      ...
      assertThat(previous.getValue()).isEqualTo(added.getValue()).isDurableWrites();
      assertThat(current.getValue()).hasName(handleId(keyspace)).isNotDurableWrites();
    }
    for (Metadata m : metadatas()) assertThat(m.getKeyspace(keyspace)).isNotDurableWrites();
  }
  ...
  /**
   * Ensures that when schema metadata is enabled using {@link
   * QueryOptions#setMetadataEnabled(boolean)} that a schema and nodelist refresh is submitted, but
   * only if schema metadata is currently disabled.
   *
   * @jira_ticket JAVA-858
   * @since 2.0.11
   */
  ...
  public void cleanup() throws InterruptedException {
    if (session1 != null) {
      ListenableFuture<List<ResultSet>> f =

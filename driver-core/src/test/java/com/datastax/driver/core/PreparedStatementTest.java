 ...
 *
 * <p>Note: this class also happens to test all the get methods from Row.
 */
...
public class PreparedStatementTest extends CCMTestsSupport {
  ...
  private List<String> createTestFixtures() {
    ...
    for (DataType keyType : primitiveTypes) {
      // This must be handled separately
      if (exclude(keyType)) continue;

      for (DataType valueType : primitiveTypes) {
        // This must be handled separately
        if (exclude(valueType)) continue;
        sb.append(", c_map_")
            .append(keyType)
            .append('_')
            .append(valueType)
    ...
    for (DataType rawType : primitiveTypes) {
      ...
      PreparedStatement ps =
          session()
              .prepare(
                  String.format(
                      "INSERT INTO %s(k, %s) VALUES ('prepared_list', ?)", ALL_LIST_TABLE, name));
      BoundStatement bs = ps.bind();
      ...
      Row row =
          session()
              .execute(
                  String.format("SELECT %s FROM %s WHERE k='prepared_list'", name, ALL_LIST_TABLE))
              .one();
      assertEquals(
          getValue(row, name, type, cluster().getConfiguration().getCodecRegistry()),
          value,
    ...
    for (DataType rawType : primitiveTypes) {
      ...
      PreparedStatement ps =
          session()
              .prepare(
                  String.format(
                      "INSERT INTO %s(k, %s) VALUES ('prepared_set', ?)", ALL_SET_TABLE, name));
      BoundStatement bs = ps.bind();
      setValue(bs, name, type, value);
      session().execute(bs);
      ...
      Row row =
          session()
              .execute(
                  String.format("SELECT %s FROM %s WHERE k='prepared_set'", name, ALL_SET_TABLE))
              .one();
      assertEquals(
          getValue(row, name, type, cluster().getConfiguration().getCodecRegistry()),
          value,
  ...
  public void prepareMapTest() {
    ...
    for (DataType rawKeyType : primitiveTypes) {
      // This must be handled separately
      if (exclude(rawKeyType)) continue;

      for (DataType rawValueType : primitiveTypes) {
        // This must be handled separately
        if (exclude(rawValueType)) continue;
        ...
        PreparedStatement ps =
            session()
                .prepare(
                    String.format(
                        "INSERT INTO %s(k, %s) VALUES ('prepared_map', ?)", ALL_MAP_TABLE, name));
        BoundStatement bs = ps.bind();
        setValue(bs, name, type, value);
        session().execute(bs);
        ...
        Row row =
            session()
                .execute(
                    String.format("SELECT %s FROM %s WHERE k='prepared_map'", name, ALL_MAP_TABLE))
                .one();
        assertEquals(
            getValue(row, name, type, cluster().getConfiguration().getCodecRegistry()),
            value,
  ...
  public void prepareWithNullValuesTest() throws Exception {
    ...
    PreparedStatement ps =
        session().prepare("INSERT INTO " + SIMPLE_TABLE2 + "(k, v) VALUES (?, ?)");
    ...
    ResultSet rs =
        session()
            .execute(
                "SELECT * FROM "
                    + SIMPLE_TABLE2
                    + " WHERE k IN ('prepWithNull1', 'prepWithNull2')");
    Row r1 = rs.one();
    Row r2 = rs.one();
  ...
  public void batchTest() throws Exception {
    ...
    try {
      ...
      PreparedStatement ps2 =
          session().prepare("INSERT INTO " + SIMPLE_TABLE2 + "(k, v) VALUES (?, 'bar')");
      ...
      bs.add(
          new SimpleStatement(
              "INSERT INTO " + SIMPLE_TABLE2 + " (k, v) VALUES ('three', 'foobar')"));
      ...
      if (cluster().getConfiguration().getProtocolOptions().getProtocolVersion()
          != ProtocolVersion.V1) throw e;
    }
  }
  ...
  public void should_fail_when_prepared_on_another_cluster() throws Exception {
    ...
    try {
      ...
      // We expect that the error gets detected without a roundtrip to the server, so use
      // executeAsync
  ...
  /**
   * Tests that, under protocol versions lesser than V4, it is NOT possible to execute a prepared
   * statement with unbound values. Note that we have to force protocol version to less than V4
   * because higher protocol versions would allow such unbound values to be sent.
   *
  ...
  public void should_not_allow_unbound_value_on_bound_statement_when_protocol_lesser_than_v4() {
    ...
    try {
      PreparedStatement ps =
          session.prepare("INSERT INTO " + keyspace + "." + SIMPLE_TABLE + " (k, i) VALUES (?, ?)");
  ...
  /**
   * Tests that, under protocol versions lesser that V4, it is NOT possible to execute a prepared
   * statement with unbound values. Note that we have to force protocol version to less than V4
   * because higher protocol versions would allow such unbound values to be sent.
   *
   * @test_category prepared_statements:binding
  ...
  /**
   * Tests that a tombstone is NOT created when a column in a prepared statement is not bound (UNSET
   * flag). This only works from protocol V4 onwards.
   *
   * @test_category prepared_statements:binding
   * @jira_ticket JAVA-777
   * @since 2.2.0
  ...
  /**
   * Tests that a value that was previously set on a bound statement can be unset by index. This
   * only works from protocol V4 onwards.
   *
   * @test_category prepared_statements:binding
  ...
  /**
   * Tests that a value that was previously set on a bound statement can be unset by name. This only
   * works from protocol V4 onwards.
   *
   * @test_category prepared_statements:binding
   * @jira_ticket JAVA-930
   * @since 2.2.0
  ...
  public void should_unset_value_by_name() {
    PreparedStatement prepared =
        session().prepare("INSERT INTO " + SIMPLE_TABLE + " (k, i) VALUES (:k, :i)");
    BoundStatement bound = prepared.bind();
  ...
  /**
   * Tests that a tombstone is created when binding a null value to a column in a prepared
   * statement.
   *
   * @test_category prepared_statements:binding
   * @jira_ticket JAVA-777
  ...
  private void assertEventsContain(QueryTrace queryTrace, String toFind) {
    for (QueryTrace.Event event : queryTrace.getEvents()) {
      if (event.getDescription().contains(toFind)) return;
    }
  ...
  public void should_propagate_idempotence_in_statements() {
    session()
        .execute(String.format("CREATE TABLE %s.idempotencetest (i int PRIMARY KEY)", keyspace));
    ...
    statement =
        new SimpleStatement(
            String.format("SELECT * FROM %s.idempotencetest WHERE i = ?", keyspace));
